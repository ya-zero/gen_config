...
#          stdin, stdout, stderr = client.exec_command("/interface bridge filter set numbers=[find chain=forward action=drop] disabled=yes")
          stdin, stdout, stderr = client.exec_command("/interface bridge filter")
          stdin, stdout, stderr = client.exec_command("add action=drop chain=forward in-bridge=br_vl_PPPoE ip-protocol=udp mac-protocol=ip src-port=5678")
          stdin, stdout, stderr = client.exec_command("add action=drop chain=forward dst-port=5678 in-bridge=br_vl_PPPoE ip-protocol=udp mac-protocol=ip")
