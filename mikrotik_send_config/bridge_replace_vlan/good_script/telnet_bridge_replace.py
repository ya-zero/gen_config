...
def send_command(host,vlan_id):
      try:
          ...
          client.connect(hostname=host,
                         ...
                         look_for_keys=False,
                         allow_agent=False)
          stdin, stdout, stderr = client.exec_command('/interface bridge port disable numbers=[find bridge =br_vl_PPPoE  interface~"ether1"]')
          time.sleep(1)
          data=stdout.read()+stderr.read()
          print ('1',data)
          stdin, stdout, stderr = client.exec_command('/interface bridge port add interface=ether1.{} bridge=br_vl_PPPoE'.format(vlan_id))
          time.sleep(1)
          data=stdout.read()+stderr.read()
          print ('2',data)
          stdin.flush()
