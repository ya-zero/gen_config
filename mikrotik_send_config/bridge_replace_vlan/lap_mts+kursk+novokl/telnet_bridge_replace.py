...
def send_command(host,vlan_id):
      ...
      try:
          ...
          client.connect(hostname=host,
                         ...
                         look_for_keys=False,
                         allow_agent=False)
          stdin, stdout, stderr = client.exec_command('/interface brid port remove numbers=[find bridge =br_vl_PPPoE  interface~"ether10"]')
          data=stdout.read()+stderr.read()
          print ('result_1_command',data)
#          stdin, stdout, stderr = client.exec_command('/interface bridge port add interface=ether1.4000.{} bridge=br_vl_PPPoE'.format(vlan_id))
#          data=stdout.read()+stderr.read()
#          print ('result_2_command',data)
          client.close()
      except:
          print ('no auth')
...
sector_replace=['192.168.30.139','192.168.30.124','192.168.30.125','192.168.30.17','192.168.30.21']
